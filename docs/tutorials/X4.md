...
3 on-board servo motor channels are available. Each is marked with letter A, B, C and can be controlled individually.  
...
Totem BUS connectors allows to attach other Totem modules and control them from X4 module. Each module is identified with number in a white square. For reference of each module check [modules section](/modules/00.info/).  
...
The IMU is a [LSM6DS3](https://www.st.com/resource/en/datasheet/lsm6ds3.pdf){target=_blank}. It is a 3-axis accelerometer and 3-axis gyroscope. Connected with ESP32 trough I2C. Can be used with any Arduino library for LSM6DS3.
...
Slow blinking - battery not detected.  
Always on - battery is charging.  
Always off - battery is charged.  

Red led next to user button for indicating board state:  
Blinking rapidly for a second - board started or indicated.  
Slow blinking - module initialization error.  
Always on - module running.  
