# [04] Code Control Board

> Integrated controller module with Bluetooth connectivity, 4 DC motor channels and 3 servo channels, Arduino programmable.

```arduino
TotemModule module(04);
```

![Module image](/assets/images/module_04.png){style="display: block;margin-left: auto;margin-right: auto;width: 50%;"}
...
Select each motor channel polarity if motor wiring is inverse.  
...
: **Description:** Set all motor channels with one command at the same moment.  
**Value:** Percentage of power 0-100%. Negative value will spin backwards.
...
**Value:** Button is pressed (true or 1) or released (false or 0).
...
```arduino
module.write("rgbAll/totem"); // Set all RGB leds to Totem colors
```
...
Calling function will reconfigure pin to digital input automatically.  
**Value write:** [LOW:HIGH] - LOW - pull to GND (0V); HIGH - pull to VCC (3.3V).  

```arduino
// Set GPIO pin A to input and pull to VCC (3.3V).
// If nothing is connected, pin will read HIGH.
module.write("gpioA/pull", HIGH);
// Set GPIO pin A to input and pull to GND (0V).
// If nothing is connected, pin will read LOW.
